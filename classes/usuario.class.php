...
class Usuario {
    ...
    public function queryInsert($dados){
        try{
            ...
            $cstLog = $con->prepare("SELECT `email`, `senha` FROM `usuario` WHERE `email` = :email AND `senha` = :senha;");
			$cstLog->bindParam(':email', $this->email, PDO::PARAM_STR);
			$cstLog->bindParam(':senha', $this->senha, PDO::PARAM_STR);
			$cstLog->execute();
